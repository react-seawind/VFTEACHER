...
import { getAllHomeWork } from '../../API/HomeWorkApi';
...
const AttendanceListing = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getAllHomeWork();
  ...
  useEffect(() => {
    const mySearch = Attendance.filter(
      (item) =>
    ...
    return (
      <div>
        <Button
          ...
          onClick={() => {
            Navigate(`/attendance/edit/${rowData.Id}`);
          }}
        />
        <Button
          icon={<FaTrash />}
  ...
  return (
    <div>
      <Breadcrumb pageName="Attendance Listing" />
